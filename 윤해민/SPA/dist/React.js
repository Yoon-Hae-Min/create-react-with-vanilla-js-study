function React() {
  ...
  const updateElement = (parent, oldNode, newNode, index = 0) => {
    ...
    if (!oldNode && newNode) {
      ...
      const addNode = newNode.cloneNode(true);
      return oldNode.remove(), parent.insertBefore(addNode, parent.children[index]);
  ...
  const reRender = () => {
    ...
    cursor = 0;
    depsCursor = 0;
    updateElement(rootElement, rootElement.childNodes[0], rootComponent());
