function React() {
  ...
  const updateAttribute = (target, newNode, oldNode) => {
    ...
    for (const [index, attribute] of newNodeObj) {
      if (attribute.name === oldNodeObj[index][1].name && attribute.value === oldNodeObj[index][1].value) {
        continue;
      }
      console.log("attribute가 바뀜");
  ...
  const reRender = () => {
    ...
    cursor = 0;
    depsCursor = 0;
    updateElement(rootElement.childNodes[0], rootElement.childNodes[0], rootComponent());
  };
